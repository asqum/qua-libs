...
# %% {Initialisation}
description = """
        JAZZ2-N CZ COUPLER AMPLITUDE CALIBRATION
This node is the coupler-only variant of 32y. The pulse sequence is the same
JAZZ2-N protocol (arXiv:2402.18926v3, Appendix I.1, Fig. 13(b)); the swept
parameter is the coupler flux-pulse amplitude scale. The control-qubit flux
pulse stays at its stored amplitude.
...
gate. The optimal coupler amplitude is the value where theta_CZ = pi, i.e.
where P_|00> is maximal.
...
Prerequisites:
    ...
    - An initial estimate of the CZ coupler amplitude.

State update:
    - qubit_pair.gates[operation].coupler_flux_pulse.amplitude
"""
...
class Parameters(NodeParameters):
    ...
    coupler_amp_range: float = 0.04
    """Half-width of the coupler amplitude-scale sweep around the stored CZ coupler amplitude (center = 1.0). Default is 0.02."""
    coupler_amp_step: float = 0.0004
    """Step of the coupler amplitude-scale sweep. Default is 0.0001."""
    N_min: int = 0
...
node = QualibrationNode(
    name="32z_cz_conditional_phase_error_amp_Jazz_2N_sweep_coupler", parameters=Parameters()
)
...
# Shared JAZZ2-N analysis reads gate_refs[...]["qubit_amplitude"] as the
# stored pulse amplitude that converts the swept scale into volts.
gate_refs = {}
for qp in qubit_pairs:
    gate_refs[qp.name] = {
        "qubit_amplitude": _coupler_amplitude(qp),
    }
...
node.namespace["sweep_axes"] = {
    ...
    "amp": xr.DataArray(amplitudes, attrs={"long_name": "coupler amplitude scale", "units": "a.u."}),
}
...
if not node.parameters.simulate:
    ...
    figures = plot_raw_data_with_fit(
        node.results["ds_fit"],
        qubit_pairs,
        title_prefix="JAZZ2-N CZ coupler amplitude calibration",
        abs_amplitude_xlabel="Coupler amplitude (V)",
    )
