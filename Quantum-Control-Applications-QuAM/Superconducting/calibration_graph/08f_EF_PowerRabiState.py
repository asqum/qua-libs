...
This sequence involves repeatedly executing the qubit pulse (EF_x180) 'N' times and
measuring the state across different qubit pulse amplitudes and number of pulses.
...
class Parameters(NodeParameters):
    ...
    operation_x180_or_any_90: Literal["EF_x180"] = "EF_x180"
...
node = QualibrationNode(name="08f_EF_Power_Rabi_State", parameters=Parameters())
...
with program() as EF_power_rabi_state:
...
if node.parameters.simulate:
    ...
    job = qmm.simulate(config, EF_power_rabi_state, simulation_config)
    ...
    with qm_session(qmm, config, timeout=node.parameters.timeout) as qm:
        job = qm.execute(EF_power_rabi_state)
    ...
    grid.fig.suptitle("EF Power Rabi State")
    plt.tight_layout()
    plt.show()
