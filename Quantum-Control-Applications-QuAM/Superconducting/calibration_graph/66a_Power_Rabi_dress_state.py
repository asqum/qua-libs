...
class Parameters(NodeParameters):
    ...
    coupler_amp: float = 0.2

node = QualibrationNode(name="66a_Power_Rabi_dress_state", parameters=Parameters())
...
coupler_amp = node.parameters.coupler_amp
            ...
            with for_(*from_array(npi, N_pi_vec)):
                with for_(*from_array(a, amps)):
                    ...
                    with for_(count, 0, count < npi, count + 1):
                        qp.qubit_control.xy.play(operation, amplitude_scale=a)
    ...
    if node.parameters.load_data_id is None:
        with node.record_state_updates():
            for qp in qubit_pairs:
                #qp.qubit_control.xy.operations['dress_x180'].amplitude = fit_results[q.name]["Pi_amplitude"]
                qp.qubit_control.xy.operations['dress_x180'] = pulses.DragCosinePulse(
                amplitude=fit_results[qp.qubit_control.name]["Pi_amplitude"],
                alpha=qp.qubit_control.xy.operations[operation].alpha,
                anharmonicity=qp.qubit_control.xy.operations[operation].anharmonicity,
                length=qp.qubit_control.xy.operations[operation].length,
                axis_angle=0,  # TODO: to check that the rotation does not overwrite y-pulses
                digital_marker=qp.qubit_control.xy.operations[operation].digital_marker,
            )
        # %% {Save_results}
