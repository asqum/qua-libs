...
class Parameters(NodeParameters):
    ...
    coupler_flux_min : float = -0.33#relative to the coupler set point
    coupler_flux_max : float =  0.45#relative to the coupler set point
        ...
        with for_(n, 0, n < n_avg, n + 1):
            ...
            with for_(*from_array(flux_coupler, fluxes_coupler)):
                with for_(*from_array(t, idle_times)):
                    ...
                    qubit.xy.play("x180")
                    qubit_pair.align()
                    qubit.z.wait(qubit.xy.operations["x180"].length)
                    qubit_pair.coupler.wait(qubit.xy.operations["x180"].length)
...
for ax, qubit in grid_iter(grid):
    ...
    if node.parameters.use_state_discrimination:
        decay_res.sel(qubit=qname).state.plot(
            ax=ax, x="flux_mV", y="idle_time",add_colorbar=False,
            cmap="viridis"
        )
        ax.set_ylabel("Idle time (µs)")
    else:
        ds.sel(qubit=qname).I.plot(
            ax=ax, x="flux_mV", y="idle_time",add_colorbar=False,
            cmap="viridis"
        )
        ax.set_ylabel("Idle time (µs)")

    ax.set_title(f"{qname} – Raw")
    ax.set_xlabel("Coupler flux (mV)")

grid.fig.suptitle("Raw vs Coupler Flux (mV)")
plt.tight_layout()
plt.show()
...
for ax, qubit in grid_iter(grid):
    ...
    fitted.sel(qubit=qname).plot(
        ax=ax, x="flux_mV", y="idle_time", add_colorbar=False,
            cmap="viridis"
    )

    ax.set_title(f"{qname} – Fit")
    ax.set_xlabel("Coupler flux (mV)")
    ax.set_ylabel("Idle time (µs)")

grid.fig.suptitle("Fit vs Coupler Flux (mV)")
plt.tight_layout()
plt.show()
...
for ax, qubit in grid_iter(grid):
    ...
    ax.set_title(f"{qname} – T1 vs Coupler Flux")
    ax.set_xlabel("Coupler flux (mV)")
    ax.set_ylabel("T1 (µs)")

grid.fig.suptitle("T1 vs Coupler Flux (mV)")
plt.tight_layout()
plt.show()
