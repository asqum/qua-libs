...
class Parameters(NodeParameters):
    ...
    coupler_bias: float = -0.05 # relative to decouple offset
    num_averages: int = 2000
    operation: str = "saturation"
    operation_amplitude_factor: Optional[float] = 0.01 #0.004, 0.02 # q6:3e-3, q7:1e-2, q8:3e-3, q9:***,
    operation_len_in_ns: Optional[int] = None
    frequency_span_in_mhz: float = 750 #12, 120
    frequency_step_in_mhz: float = 2.5 #0.1, 1
    frequency_shift_in_mhz: float = 0 #0  
    min_flux_offset_in_v: float = 0.15 ##-0.042
    max_flux_offset_in_v: float = 0.25 #0.042
    num_flux_points: int = 101
    flux_point_joint_or_independent: Literal["joint", "independent"] = "joint"
    simulate: bool = False
    simulation_duration_ns: int = 2500
    timeout: int = 100
    driving_LO_GHz:float = 3.5
    load_data_id: Optional[int] = None
    multiplexed: bool = False


node = QualibrationNode(name="03e_Qubit_Spectroscopy_vs_Flux", parameters=Parameters())
...
qp = machine.qubit_pairs[node.parameters.coupler]
...
additional_flux_settle_time:int = 800 # QUA clock cycles
# original_LOs, original_bands = {}, {}
# def get_band(freq):
#     if 50e6 <= freq < 5.5e9:
#         return 1
...
#     for q in qubits:
#         original_bands[q.name] = q.xy.opx_output.band
#         q.xy.opx_output.band = get_band(node.parameters.driving_LO_GHz * 1e9)
#         original_LOs[q.name] = q.xy.opx_output.upconverter_frequency
#         q.xy.opx_output.upconverter_frequency = int(node.parameters.driving_LO_GHz * 1e9)
...
coupler_flux_bias = node.parameters.coupler_bias
...
with program() as multi_qubit_spec_vs_flux:
    ...
    xy_wait_time = declare(int, value=additional_flux_settle_time//2)  # QUA variable for the xy wait time
        ...
        with for_(n, 0, n < n_avg, n + 1):
            ...
            with for_(*from_array(df, dfs)):
                ...
                with for_(*from_array(dc, dcs)):
                    ...
                    # Bring the qubit and coupler to the desired point during the saturation pulse
                    qp.coupler.play("const", amplitude_scale=coupler_flux_bias / qp.coupler.operations["const"].amplitude, duration=duration+additional_flux_settle_time)
                    qubit.z.play("const", amplitude_scale=dc / qubit.z.operations["const"].amplitude, duration=duration+additional_flux_settle_time)
                    # qp.coupler.play("const", amplitude_scale=dc / qubit.z.operations["const"].amplitude, duration=duration)
                    # Apply saturation pulse to all qubits
                    fixed_qubit.xy.wait(xy_wait_time)
                    fixed_qubit.xy.play(
                        operation,
                        amplitude_scale=operation_amp,
                        duration=duration+xy_wait_time,
                    )
                    qubit.align()
...
if node.parameters.simulate:
    ...
    simulation_config = SimulationConfig(duration=(node.parameters.simulation_duration_ns * 4)//4)  # In clock cycles = 4ns
    ...
    if node.parameters.load_data_id is None:
        with node.record_state_updates():
            for q in qubits:
                ...
                q.xy.opx_output.upconverter_frequency = original_LOs[q.name]
                q.xy.opx_output.band = original_bands[q.name]
