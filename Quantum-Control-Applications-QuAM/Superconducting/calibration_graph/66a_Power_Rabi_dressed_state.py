...
Prerequisites:
    - Having found the strong couping point of qubit and coupler.
    
Next steps before going to the next node:
    - Update the dressed state pulse amplitude in the state.
...
class Parameters(NodeParameters):
    ...
    dressed_inter_amp: float = 0.2
    dressed_inter_freq: int = 100e6

node = QualibrationNode(name="66a_Power_Rabi_dressed_state", parameters=Parameters())
...
#a0*t0=an*tn => an=a0*t0/tn

for qp in qubit_pairs:
    qp.qubit_control.xy.operations['dressed_x180'] = pulses.DragCosinePulse(
    amplitude=qp.qubit_control.xy.operations['x180'].amplitude*qp.qubit_control.xy.operations['x180'].length/200,
    alpha=qp.qubit_control.xy.operations['x180'].alpha,
    anharmonicity=qp.qubit_control.xy.operations['x180'].anharmonicity,
    length=int(200),
    axis_angle=0,  # TODO: to check that the rotation does not overwrite y-pulses
    digital_marker=qp.qubit_control.xy.operations['x180'].digital_marker,
)
config = machine.generate_config()
...
coupler_amp = node.parameters.dressed_inter_amp
dressed_inter_freq = node.parameters.dressed_inter_freq
...
with program() as power_rabi:
        ...
        with for_(n, 0, n < n_avg, n + 1):
            ...
            with for_(*from_array(npi, N_pi_vec)):
                with for_(*from_array(a, amps)):
                    qp.qubit_control.xy.update_frequency(dressed_inter_freq)
                    qp.coupler.set_dc_offset(coupler_amp)
                    # Initialize the qubits
                    if reset_type == "active":
                        active_reset(qp.qubit_control, "readout")
                    else:
                        if node.parameters.simulate: qp.qubit_control.wait(16 * u.ns)
                        # else: qubit.wait(qubit.thermalization_time * u.ns)
                        else: qp.qubit_control.wait(machine.thermalization_time * u.ns)

                    # Loop for error amplification (perform many qubit pulses)
                    with for_(count, 0, count < npi, count + 1):
                        qp.qubit_control.xy.play('dressed_x180', amplitude_scale=a)
                    qp.qubit_control.align()
                    qp.qubit_control.resonator.measure("readout", qua_vars=(I[i], Q[i]))
                    if state_discrimination:
                        assign(state[i], I[i] > qp.qubit_control.resonator.operations["readout"].threshold)
    ...
    with stream_processing():
        n_st.save("n")
        for i, qubit_pairs in enumerate(qubit_pairs):
            if operation == "x180":
                if state_discrimination:
...
if not node.parameters.simulate:
    if node.parameters.load_data_id is None:
        ...
        qubit_pairs = [machine.qubit_pairs[qp] for qp in node.parameters.qubit_pairs]
        qubits = [qubit_pair.qubit_control for qubit_pair in qubit_pairs]
        ...
        ds = ds.assign_coords(
        {
            "abs_amp": (
                ["qubit", "amp"],
                np.array([qp.qubit_control.xy.operations[operation].amplitude * amps for qp in qubit_pairs]),
            )
            }
        )
    else:
    ...
    if node.parameters.load_data_id is None:
        with node.record_state_updates():
            for qp in qubit_pairs:
                qp.qubit_control.xy.operations['dressed_x180'].amplitude =fit_results[qp.qubit_control.name]["Pi_amplitude"]
        # %% {Save_results}
