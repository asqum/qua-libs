...
node = QualibrationNode(name="03d_Qubit_Spectroscopy_vs_Coupler_Flux", parameters=Parameters())
#node_id = get_node_id()
            ...
            with for_(*from_array(df, dfs)):
                ...
                with for_(*from_array(dc, dcs)):
                    ...
                    qubit_pair.coupler.play(
                        "const", 
                        amplitude_scale = dc / qubit_pair.coupler.operations["const"].amplitude, 
                        duration = duration + 200)
                    # let the flux settle
                    qubit.xy.wait(200)
                    qubit.xy.play(
...
if not node.parameters.simulate:
    ...
    for ax, qubit in grid_iter(grid):
        ...
        ax.set_xlabel("Coupler Flux (mV)")  # <— label updated
