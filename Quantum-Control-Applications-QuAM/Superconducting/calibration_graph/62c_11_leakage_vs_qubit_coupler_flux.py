...
class Parameters(NodeParameters):
    ...
    """If provided, loads data from a previous experiment with this ID instead of running a new one."""
    coupler_flux_min : float = -0.095 #relative to the coupler set point
    """Minimum of the coupler flux sweep"""
    coupler_flux_max : float =  -0.070 #relative to the coupler set point
    """Maximum of the coupler flux sweep for"""
    coupler_flux_num_points : int = 101
...
node = QualibrationNode(name="62c_11_leakage_vs_qubit_coupler_flux", parameters=Parameters())
assert not (
...
with program() as CPhase_Oscillations:
    ...
    with stream_processing():
        ...
        for i in range(num_qubit_pairs):
            state_st_control[i].buffer(len(fluxes_qubit)).buffer(len(fluxes_coupler)).buffer(
                n_avg
            ).save(f"state_control{i + 1}")
            state_st_target[i].buffer(len(fluxes_qubit)).buffer(len(fluxes_coupler)).buffer(
                n_avg
...
P11 = ((sc == 1) & (st == 1)).astype(int).mean("N").rename("P11")
...
for qp in qubit_pairs:
    ...
    flux_coupler_min = float(P11.flux_coupler.values[i_cpl_min])
    flux_qubit_min = float(P11.flux_qubit_full.values[i_qb_min])
    flux_coupler_full_min = float(flux_cpl_full.interp(flux_coupler=flux_coupler_min))
...
if not node.parameters.simulate:
    ...
    for ax, qp in grid_iter(grid):
        ...
        values_to_plot = ds["P11"].sel(qubit=qp_name)
        # Coordinates in mV
        ...
        ax.set_title(
            f"{qp_name}, Decoupling offset = {qubit_pair.coupler.decouple_offset * 1e3 :.0f} mV ", fontsize=9
        )
        if legend_entries:
            ax.legend(fontsize=7, loc="upper right", frameon=True)

        # overall title per qubit pair
        grid.fig.suptitle(f"Lekage out of 11 state \n for {operation_name} pulse", y=0.97, fontsize=12)
        plt.tight_layout()
        plt.show()
    ...
    if not node.parameters.simulate:
        with node.record_state_updates():
            for qp in qubit_pairs:
                ...
                qp.gates[operation_name].coupler_flux_pulse.amplitude = node.results["results"][qp.name][
                    "flux_coupler_max"
                ]
# %% {Save_results}
if not node.parameters.simulate:
