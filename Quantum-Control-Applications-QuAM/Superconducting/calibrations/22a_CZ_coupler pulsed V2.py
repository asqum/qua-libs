...
# Ad qubit pulses
q1.z.operations["flux_pulse"] = pulses.SquarePulse(length=100, amplitude=0.5)
coupler.operations["flux_pulse"] = pulses.SquarePulse(length=100, amplitude=0.5)
    ...
    with for_(n, 0, n < n_avg, n + 1):
        ...
        with for_(*from_array(t, ts)):
            ...
            with for_(*from_array(dc, dcs)):
                ...
                q1.z.play("flux_pulse", duration=t, amplitude_scale=2*scale * dc)
                coupler.play("flux_pulse", duration=t, amplitude_scale=2*dc)
                # play("flux_pulse" * amp(2*dc), coupler.name, duration=t)
                wait(t)
