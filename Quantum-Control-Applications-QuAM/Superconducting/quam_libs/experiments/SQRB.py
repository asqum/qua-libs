    ...
    if not node.parameters.simulate:
        ...
        for ax, qubit in grid_iter(grid):
            ...
            ax.text(
                0.0,
                1.07,
                f"Native Gate fidelity = {(1 - EPG.sel(**qubit).values):.1%}",
                transform=ax.transAxes,
            )
            info_text = (
                f"Clifford error: {EPC.sel(**qubit).values:.1e}\n"
                f"Randomness per depth: {node.parameters.num_random_sequences}"
            )

            # 由於 RB decay 曲線通常是左上到右下，有時放右上角 (0.95, 0.95) 會是最空曠的完美位置
            ax.text(
                0.95, 0.95,  
                info_text,
                transform=ax.transAxes,
                fontsize=9,
                verticalalignment='top',    # 改為靠上對齊
