...
class CP_T1_EXP(ExpTemplate):
            ...
            for i, c_name in enumerate(self.CRD.paired_elements):
                ...
                with for_(n, 0, n < self.node.parameters.num_averages, n + 1):
                    ...
                    with for_(*from_array(t, self.variables['idle_time'])):
                        ...
                        wait(4)
                        align()
                        if not self.node.parameters.debug:
    ...
    # plot
    def visualize(self, histogram_outlier_percentage:float=0):
        '''
            ...
            if update_state_also:
                with self.node.record_state_updates():
                    ...
                    for c in self.CRD.get_all_TransmonPairs():
                        c.extras["T1"] = float(self.analyzed_items['mu_collection'][c.name]) * 1e-6
                        c.extras["T1_dev"] = float(self.analyzed_items['sig_collection'][c.name]) * 1e-6
