...
node = QualibrationNode( name="06x_coupler_T2echo", parameters=Parameters())

class CP_T2_EXP(ExpTemplate):
            ...
            for i, c_name in enumerate(self.CRD.paired_elements):
                ...
                with for_(n, 0, n < self.node.parameters.num_averages, n + 1):
                    ...
                    with for_(*from_array(t, self.variables['idle_time'])):
                        ...
                        self.CRD.get_obj_with_type(c_name,"drive_q").wait(t)
                        self.CRD.get_obj_with_type(c_name,"drive_q").xy.play(f"x90_{c_name}", amplitude_scale=-1.0)
                        align()
    ...
    def dataset_post_proccess(self):
        ...
        ds = ds.assign_coords(idle_time= 8 * ds.idle_time / u.us)
    ...
    def visualize(self, histogram_outlier_percentage:float=1):
        ...
        if not self.node.parameters.simulate:
            ...
            for ax, qubit in grid_iter(grid):
                ...
                if self.node.parameters.histo_num > 1:
                    ...
                    if histogram_outlier_percentage > 0 and histogram_outlier_percentage<100:
                        lower_bound = np.percentile(data, histogram_outlier_percentage)   
                        upper_bound = np.percentile(data, 100-histogram_outlier_percentage)  
            ...
            plt.suptitle(f"Coupler T2 with {self.node.parameters.reset_type} reset")
            plt.tight_layout()
            plt.show()

            self.node.results["t2_stats"] = {q: {"mu_us": mu_collection[q], "sigma_us": sig_collection[q]} for q in list(self.analyzed_items['t1s'].keys())}
            ...
            if update_state_also:
                with self.node.record_state_updates():
                    for c in self.CRD.get_all_TransmonPairs():
                        c.extras["T2"] = float(self.analyzed_items['mu_collection'][c.name]) * 1e-6
                        c.extras["T2_dev"] = float(self.analyzed_items['sig_collection'][c.name]) * 1e-6
