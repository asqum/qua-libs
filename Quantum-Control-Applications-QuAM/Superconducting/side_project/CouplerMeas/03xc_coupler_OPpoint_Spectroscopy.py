...
class Parameters(NodeParameters):
    ...
    arbitrary_flux_bias: float = 0.116
    operation: str = "saturation"
    operation_amplitude_factor: Optional[float] = 0.05    # 0.05 , 0.1 good
    operation_len_in_ns: Optional[int] = None
    Driving_LO_GHz:float|None = 3.9      # None use state recorded. Otherwise, use this value as the new LO (and will be updated into state)
...
node = QualibrationNode(name="03xc_Coupler_OperationPoint_Spectroscopy", parameters=Parameters())
...
if node.parameters.load_data_id is None and not node.parameters.simulate:
    ...
    if "aswap_supplier" in coupler[0].extras["RD"] and coupler[0].extras["RD"]["aswap_supplier"].lower() == 'c':
...
if node.parameters.arbitrary_flux_bias is None:
    arb_flux_bias_offset = {c.name: c.gates["Cz"].coupler_flux_pulse.amplitude for c in coupler}
else:
    arb_flux_bias_offset = {c.name: node.parameters.arbitrary_flux_bias for c in coupler}
    ...
    for i, qubit in enumerate(drive_q):
        ...
        with for_(n, 0, n < n_avg, n + 1):
            ...
            with for_(*from_array(df, dfs)):
                ...
                coupler[0].coupler.play("const", amplitude_scale=arb_flux_bias_offset[coupler[0].name] / coupler[0].coupler.operations["const"].amplitude, duration=duration)
                qubit.xy.play(
                    operation,
...
if not node.parameters.simulate:
        ...
        ds = ds.assign_coords(
            {
                "freq_full": (
                    ["qubit", "freq"],
                    np.array([dfs + coupler[0].extras["RD"]["IF"] + LO_to_plot for q in drive_q]),
                )
            }
        )
    ...
    for ax, qubit in grid_iter(grid):
        ...
        ax.set_title(f"{node.parameters.couplers}\n @{node.parameters.arbitrary_flux_bias:.3f}V")
