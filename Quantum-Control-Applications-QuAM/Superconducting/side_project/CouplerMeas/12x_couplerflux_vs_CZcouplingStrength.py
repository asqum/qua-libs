...
# %% {Node_parameters}
qubit_pair_indexes = 5
class Parameters(NodeParameters):

    qubit_pairs: Optional[List[str]] =  ["coupler_q%s_q%s"%(qubit_pair_indexes,qubit_pair_indexes+1)]
    ...
    coupler_flux_max : float = 0.4
    coupler_flux_step : float = 0.004


    idle_time_min : int = 16
    idle_time_max : int = 416
...
if not node.parameters.simulate:
    ...
    target_coupling_strength_MHz = round(1e3/(qubit_pairs[0].gates[node.parameters.operation].coupler_flux_pulse.length*u.ns),1)
...
if not node.parameters.simulate:
    ...
    for ax, qp in grid_iter(grid):
        ...
        ax.set_ylim(0, 20)
        ax.set_xlim(0, 0.35)
    grid.fig.suptitle(f'g={target_coupling_strength_MHz} MHz at flux pulse amplitude = {round(float(coupler_flux_pulse.sel(qubit=qp["qubit"]).values),3)} V')
    plt.tight_layout()
    plt.show()
...
if not node.parameters.simulate:
    with node.record_state_updates():
        for qp in qubit_pairs:
            ...
            qp.extras["CZgMHz_bias_conversion"][str(target_coupling_strength_MHz)] = float(coupler_flux_pulse.sel(qubit=qp.name).values)
# %% {Save_results}
if not node.parameters.simulate:    
