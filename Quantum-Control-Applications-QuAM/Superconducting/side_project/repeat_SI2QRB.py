...
from batched_SI2QRB import run_batched_rb
...
random_circuit_num: int = 100
cooldown_sec = 5 #Change
gate_time_ns = 40
waveform = "test"

# folder path to save the file
out_dir = Path('/home/ratiswu/Qualibrate_data/as-qpu-10Q9Cv2/CZ_DuraSimuCompa') #Change
out_dir.mkdir(parents=True, exist_ok=True)

# csv file name (contains the time to avoid overlap)
session_ts = datetime.now().isoformat(timespec="seconds").replace(":", "-")
wide_csv = out_dir / f"CZ_{waveform}_{gate_time_ns}ns_{session_ts}.csv" 
i = 0
header_qubits = None  
while True:
    i += 1
    ...
    try:
        fluctuations, qubit_names = run_batched_rb(target_operation= 'cz', total_circuits= random_circuit_num)
