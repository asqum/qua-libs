...
from quam_libs.lib.fit import fit_oscillation_decay_exp, decay_exp
...
if not node.parameters.simulate:
    ...
    for q_name in qbs:
        for iter_val in iterations:
            ...
            if node.parameters.use_state_discrimination:
                fit_data = fit_oscillation_decay_exp(ds_sub.state, "idle_time")
            else:
                fit_data = fit_oscillation_decay_exp(ds_sub.I, "idle_time")
    ...
    node.results["t2*_stats"] = {q: {"mu_us": mu_collection[q], "sigma_us": sig_collection[q]} for q in qbs}
