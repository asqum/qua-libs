...
class Parameters(NodeParameters):
    ...
    operation_amplitude_factor: Optional[float] = 0.07  #0.004, 0.0004
    frequency_span_in_mhz: float = 700 #200, 4, 800
    frequency_step_in_mhz: float = 7 #0.25, 0.01
...
node = QualibrationNode(name="03ax_Overlap2tone", parameters=Parameters())
...
if not node.parameters.simulate:
    ...
    if node.parameters.load_data_id is None:
        with node.record_state_updates():
            for q in qubits:
                if not np.isnan(result.sel(qubit=q.name).position.values):
                    ...
                    # Only this node edit saturation amplitude
                    if used_amp<=1 :
                        q.xy.operations["saturation"].amplitude = used_amp
        ...
        # %% {Save_results}
        for q in qubits:
            # q.xy.operations['saturation'].length = original_paras[q.name]['saturation_length']
