...
if node.parameters.z_source.startswith('q'):
    source = machine.qubits[node.parameters.z_source]
    z_sources = source.z 
elif node.parameters.z_source.startswith('c'):
    source = machine.qubit_pairs[node.parameters.z_source]
    z_sources = source.coupler
else:
    raise ValueError(f"Can not match the z source element named '{node.parameters.z_source}' in machine.")
...
fluxes = np.arange(
    node.parameters.flux_min, node.parameters.flux_max, step=node.parameters.flux_step
)
        ...
        with for_(n, 0, n < n_avg, n + 1):
            ...
            with for_(*from_array(flux, fluxes)):
                with for_(*from_array(t, idle_times)):
                    ...
                    # end of compensation
                    wait(20)
                    qubit.z.play("const", amplitude_scale = comp / qubit.z.operations["const"].amplitude,  duration=t)
                    z_sources.play("const", amplitude_scale= flux / z_sources.operations["const"].amplitude, duration=t)
                    wait(20)
                    align()
...
if not node.parameters.simulate:
    ...
    for ax, qubit in grid_iter(grid):
        ...
        ax.set_title(f'{qubit["qubit"]}, a = {round(abs(fitvals.sel(qubit=qubit["qubit"], degree=2).polyfit_coefficients.item()), 3)}')
        ax.set_xlabel(" Z source Flux amplitude (V)")
