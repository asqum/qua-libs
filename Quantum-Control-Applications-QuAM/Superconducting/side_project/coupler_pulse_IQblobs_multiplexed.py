...
with program() as iq_blobs:
        ...
        with for_(*from_array(dc, dcs)):
            ...
            for i, qp in enumerate(qubit_pairs):
                qp.coupler.play(
                    ...
                    duration = 1000//4 + xy_dura//4
                )
            
            # align()

            for i, qubit in enumerate(qubits):
                qubit.xy.wait(1000//4)
                qubit.xy.play("x180")
    ...
    with stream_processing():
        ...
        for i in range(num_qubits):
            I_g_st[i].buffer(len(dcs)).buffer(n_runs).save(f"I_g{i + 1}")
            Q_g_st[i].buffer(len(dcs)).buffer(n_runs).save(f"Q_g{i + 1}")
            I_e_st[i].buffer(len(dcs)).buffer(n_runs).save(f"I_e{i + 1}")
            Q_e_st[i].buffer(len(dcs)).buffer(n_runs).save(f"Q_e{i + 1}")
...
if not node.parameters.simulate:
    if node.parameters.load_data_id is None:
        ...
        ds = fetch_results_as_xarray(job.result_handles, qubits, {"amplitude": dcs, "N": np.linspace(1, n_runs, n_runs)})
    ...
    for ax, qubit in grid_iter(grid):
        ...
        ax.set_xlabel("Real coupler voltage (V)")
