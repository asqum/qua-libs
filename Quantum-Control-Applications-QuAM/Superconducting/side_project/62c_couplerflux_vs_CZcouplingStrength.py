...
class Parameters(NodeParameters):
    ...
    coupler_flux_min : float = -0.2
    coupler_flux_max : float = 0
    coupler_flux_step : float = 0.001

    idle_time_min : int = 16
    idle_time_max : int = 296
    idle_time_step : int = 4

    use_state_discrimination: bool = True
    readout_flip:bool = False
...
if not node.parameters.simulate:
    ...
    target_coupling_strength_MHz = 12.25
...
if not node.parameters.simulate:
    ...
    for ax, qp in grid_iter(grid):
        ...
        ax.set_ylim(0, target_coupling_strength_MHz*1.5)
