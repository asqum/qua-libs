...
def triangle_peak(t, amp, t0, half_width, offset):
    ...
    Both XY and flux pulses have the same length by construction
...
def fit_delay_trace(da, xy_duration: float):
    ...
    # Only consider samples far from the peak (flux fully misaligned)
    wing_mask = np.abs(x - t0_guess) > xy_duration
    # Noise from wings (far from peak, flux fully misaligned); fall back to
...
def fit_routine(da, node):
    qubit_coord = da.qubit.item()
    qubit = next(
        q for q in node.namespace["qubits"] if q.id == qubit_coord or q.name == qubit_coord
    )
