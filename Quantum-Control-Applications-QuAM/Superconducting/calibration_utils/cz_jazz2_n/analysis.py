"""Analysis module for the JAZZ2-N CZ amplitude calibration.
...
Averaging over all probed N collapses the N-dependence into the central lobe
of a sinc-like pattern centred on the optimal amplitude. We fit
...
class FitResults:
    ...
    success: bool
    """Whether a finite optimum was found (sinc fit, or parabolic fallback)."""
    fit_method: str = "sinc"
...
def fit_raw_data(ds: xr.Dataset, node: QualibrationNode) -> Tuple[xr.Dataset, Dict[str, FitResults]]:
    ...
    for qp_name in qp_names:
        ...
        if "amp_full" in ds.coords and amp_values[0] != 0:
            stored_amp = float(ds["amp_full"].sel(qubit_pair=qp_name).values[0] / amp_values[0])
        else:
            qp = next(qp for qp in qubit_pairs if qp.name == qp_name)
            if qp.name in gate_refs:
                stored_amp = gate_refs[qp.name]["qubit_amplitude"]
            else:
                stored_amp = qp.gates[operation].flux_pulse_control.amplitude
